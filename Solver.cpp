...
int main()
{
    cout << "HERE";
    Experiments run;
    string MSG = "Welcome!\nPlease navigate to \'Data\\Custom\' folder and fill the \'experiment.csv\' file according to the example.\nPress ENTER to confirm and start the experiment\nPress ESC to quit\n";
    
    system("Cls");
    cout << MSG;
    char input = getch();

        while (input != KEY_ESC && input != KEY_ENT)
        {
            if (input == KEY_ENT)
            {
                system("cls");
